# 🚁 DQN无人机移动控制 - 快速指南

## 📖 这是什么？

这是一个使用**深度强化学习（DQN）**训练无人机自主移动和区域扫描的系统。无人机通过学习在6个方向（上下左右前后）移动，来完成扫描任务。

## 🎯 核心特点

| 特性 | 说明 |
|-----|------|
| **动作空间** | 6个离散动作（上/下/左/右/前/后） |
| **观察空间** | 21维状态（位置、速度、熵值、Leader信息等） |
| **训练模式** | 支持纯模拟训练 & AirSim集成训练 |
| **灵活配置** | JSON配置文件，可自定义奖励和参数 |

## 🚀 5分钟快速开始

### Step 1: 安装依赖

```bash
pip install torch stable-baselines3 numpy gym
```

### Step 2: 开始训练（纯模拟）

**Windows**:
```bash
train_movement_dqn.bat
```

**Linux/Mac**:
```bash
python multirotor/DQN/train_movement_dqn.py
```

### Step 3: 测试模型

训练完成后（10万步，约10-30分钟）：

```bash
# Windows
test_movement_dqn.bat

# Linux/Mac
python multirotor/DQN/test_movement_dqn.py
```

## 📂 文件说明

```
DQN/
├── 📄 movement_env.py              # 环境定义（核心）
├── ⚙️ movement_dqn_config.json    # 配置文件
├── 🎓 train_movement_dqn.py       # 纯模拟训练
├── 🎮 train_movement_with_airsim.py # AirSim训练
├── 🧪 test_movement_dqn.py        # 模型测试
└── 📚 MOVEMENT_DQN.md             # 完整文档
```

## 🎮 两种训练模式

### 模式1: 纯模拟训练（推荐新手）

**特点**:
- ⚡ 快速启动
- 💻 无需Unity
- 🔄 适合调试

**使用**:
```bash
train_movement_dqn.bat
```

### 模式2: AirSim集成（真实环境）

**特点**:
- 🎯 真实反馈
- 🎮 需要Unity
- 📈 更准确

**使用**:
```bash
# 1. 启动Unity客户端
# 2. 运行训练
train_movement_with_airsim.bat
```

## ⚙️ 配置调整

编辑 `movement_dqn_config.json`:

```json
{
  "movement": {
    "step_size": 1.0,      // 移动步长（米）
    "max_steps": 500       // 最大步数
  },
  "rewards": {
    "exploration": 10.0,   // 探索奖励
    "collision": -50.0     // 碰撞惩罚
  },
  "training": {
    "total_timesteps": 100000,  // 训练步数
    "learning_rate": 0.0001     // 学习率
  }
}
```

## 📊 监控训练

使用Tensorboard查看训练曲线：

```bash
tensorboard --logdir=multirotor/DQN/logs/movement_dqn/
```

然后访问 `http://localhost:6006`

## 🎯 期望结果

训练成功的指标：
- ✅ **平均奖励** > 100
- ✅ **扫描完成率** > 90%
- ✅ **碰撞次数** < 5次/episode

## 🔧 常见问题

### Q: 训练很慢？
**A**: 使用GPU版本PyTorch或减少 `total_timesteps`

### Q: 模型效果不好？
**A**: 
1. 增加训练时间
2. 调整奖励权重
3. 检查探索率设置

### Q: 找不到模型文件？
**A**: 模型保存在 `multirotor/DQN/models/` 目录

## 📚 更多信息

详细文档请查看: [MOVEMENT_DQN.md](./MOVEMENT_DQN.md)

## 🎓 工作原理

```
┌─────────────┐
│   环境状态   │ (21维观察)
│  位置/速度   │
│  熵值/Leader │
└──────┬──────┘
       │
       v
┌─────────────┐
│  DQN网络    │ (神经网络)
│   Q值计算   │
└──────┬──────┘
       │
       v
┌─────────────┐
│   选择动作   │ (0-5: 6方向)
│  上下左右前后 │
└──────┬──────┘
       │
       v
┌─────────────┐
│  执行移动   │
│  获得奖励   │
└─────────────┘
```

## 📈 训练流程

1. **初始化**: 随机探索环境
2. **收集经验**: 保存到经验回放缓冲区
3. **学习更新**: 从缓冲区采样训练
4. **探索衰减**: 逐渐减少随机探索
5. **策略优化**: 学习最优移动策略

## 🌟 下一步

1. ✅ 完成基础训练
2. 🎯 调优参数获得更好性能
3. 🎮 集成到AirSim环境
4. 🚀 部署到实际无人机任务

---

**💡 提示**: 第一次训练建议使用默认参数，观察效果后再调整。

**🤝 支持**: 遇到问题请查看详细文档或提Issue。


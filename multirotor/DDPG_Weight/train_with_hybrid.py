...
def _setup_hybrid_config(config_file: str, mirror_drones: list) -> str:
    ...
    for drone_name in drones_config.get_all_drones():
        ...
        if drone_info:
            ...
            print(f"   [OK] {drone_name}: isCrazyflieMirror = {is_mirror}")
    ...
    print(f"   [S] 临时配置文件: {temp_config_file}")
...
def _restore_config(original_config: str, temp_config: str):
    ...
    if temp_config != original_config and os.path.exists(temp_config):
        try:
            ...
            print(f"[OK] 已清理临时配置文件: {temp_config}")
        ...
        if (
            ...
            if hasattr(self, "data_logger") and self.data_logger:
                self.data_logger.record_episode_stats(
                    episode=self.episode_count, reward=ep_reward, length=ep_length
                )
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="实体镜像无人机列表（如: UAV1 UAV2）",
    )
    parser.add_argument(
        "--initial-model-path", type=str, default=None, help="初始模型路径"
    )
    parser.add_argument(
        "--use-initial-weights",
        action="store_true",
    ...
    parser.add_argument(
        "--model-name", type=str, default=None, help="指定模型名称（不含.zip）"
    )
    ...
    total_timesteps = int(
        _get_config_value(None, config, "total_timesteps", DEFAULT_TOTAL_TIMESTEPS)
    )
    step_duration = float(
    ...
    mirror_drones = (
        args.mirror_drones
        if args.mirror_drones is not None
        else _get_config_value(None, config, "mirror_drones", DEFAULT_MIRROR_DRONES)
    )
    ...
    model_name = _get_config_value(
        args.model_name, config, "model_name", "weight_predictor_hybrid"
    )
    # ==========================================
    ...
    if mirror_drones:
        print(f"[Link] 实体镜像无人机: {', '.join(mirror_drones)}")
        print(f"   (这些无人机将使用实体Crazyflie的状态数据)")
    else:
        print(f"[Link] 实体镜像无人机: 无 (纯虚拟训练)")
    ...
    try:
        ...
        _tmp_vis_log_dir = os.path.join(
            os.path.dirname(__file__), "logs", "ddpg_hybrid"
        )
        ...
        print(f"[OK] 环境创建成功")
        print(f"  [Mode] 模式: 虚实融合训练")
        print(f"  [Train] 训练无人机: {drone_names[0]}")
        if mirror_drones:
            print(f"  [Link] 实体镜像: {', '.join(mirror_drones)}")
        print(f"  [Clock]  每步时长: {step_duration}秒")
        ...
        final_model_path = os.path.join(
            model_dir, f"weight_predictor_hybrid_{timestamp}"
        )

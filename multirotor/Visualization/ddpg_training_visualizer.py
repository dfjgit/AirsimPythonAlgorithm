...
class DDPGTrainingVisualizer(BaseVisualizer):
    ...
    def __init__(self, server=None, env=None):
        super().__init__(server=server, env=env, window_title="DDPG训练实时可视化")
    ...
    def setup_panels(self):
        ...
        # 重置信息面板
        reset_info_panel = ResetInfoPanel(width=370, height=200)
        self.panel_manager.register_panel(reset_info_panel, position="auto")
    ...
    def get_visualization_data(self) -> Dict[str, Any]:
        ...
        if cts:
            ...
            data["current_episode_reward"] = float(
                cts.get("current_episode_reward", 0.0)
            )
            ...
            if self.episode_rewards:
                data["avg_reward"] = sum(self.episode_rewards) / len(
        ...
        should_collect_weights = (
            current_time - self._last_weight_collection_time
        ) >= self._weight_collection_interval

        if (
            self.server
            and hasattr(self.server, "drone_names")
            and self.server.drone_names
        ):
            first_drone = self.server.drone_names[0]
            if first_drone in self.server.algorithms:
                try:
                    weights = self.server.algorithms[
                        first_drone
                    ].get_current_coefficients()
                    if weights:
                        data["weights"] = weights
                        data["use_dqn"] = getattr(
                            self.server, "use_learned_weights", True
                        )
                        # 定期更新权重历史（避免每帧都更新）
        ...
        if self.server:
            # 兼容两种模式：
            # - AlgorithmServer: _reset_history (带下划线)
            # - SnapshotServerProxy: reset_history (不带下划线)
            if hasattr(self.server, "_last_reset_reason"):
                data["last_reset_reason"] = self.server._last_reset_reason
            elif hasattr(self.server, "last_reset_reason"):
                data["last_reset_reason"] = self.server.last_reset_reason

            if hasattr(self.server, "_last_reset_time"):
                data["last_reset_time"] = self.server._last_reset_time
            elif hasattr(self.server, "last_reset_time"):
                data["last_reset_time"] = self.server.last_reset_time

            if hasattr(self.server, "_reset_history"):
                data["reset_history"] = list(self.server._reset_history)
            elif hasattr(self.server, "reset_history"):
                data["reset_history"] = list(self.server.reset_history)
    ...
    def generate_training_charts(
        ...
        try:
            ...
            fig.suptitle(
                f"🎯 训练统计分析 (Episodes: {self.episode_count})",
                fontsize=16,
                fontweight="bold",
            )
            ...
            if preview_before_save:
                ...
                if not auto_save:
                    print("\n💾 是否保存图表?")
                    response = (
                        input("输入 'y' 或 'yes' 保存,其他任意键取消: ").strip().lower()
                    )
                    ...
                    output_path = os.path.join(
                        log_dir, f"training_charts_{time.strftime('%Y%m%d_%H%M%S')}.png"
                    )
                    fig.savefig(output_path, dpi=150, bbox_inches="tight")
                    saved_files.append(output_path)

...
class RewardCurvePanel(BasePanel):
    ...
    def draw(self, screen: pygame.Surface, data: Dict[str, Any]):
        ...
        y_offset = self.draw_title(screen, f'奖励曲线 (Episode {displayed_count})', accent)

        if not reward_history or len(reward_history) < 2:
            hint_text = self._font.render('等待训练数据...', True, self.TEXT_MUTED)
        ...
        pygame.draw.line(screen, self.PANEL_BORDER, (chart_origin_x, chart_origin_y), (chart_origin_x + chart_width, chart_origin_y), 2)
        pygame.draw.line(screen, self.PANEL_BORDER, (chart_origin_x, chart_origin_y - chart_height), (chart_origin_x, chart_origin_y), 2)
        ...
        if len(points) > 1:
            ...
            for point in points[-min(4, len(points)):]:
                pygame.draw.circle(screen, accent, point, 3)

        avg_reward = sum(rewards) / len(rewards)
        avg_y = chart_origin_y - ((avg_reward - min_reward) / reward_range) * chart_height
        pygame.draw.line(screen, self.WARNING, (chart_origin_x, int(avg_y)), (chart_origin_x + chart_width, int(avg_y)), 1)

        label_max = self._small_font.render(f'{max_reward:.1f}', True, self.TEXT_PRIMARY)
        screen.blit(label_max, (self.x + 5, chart_origin_y - chart_height - 5))
        label_min = self._small_font.render(f'{min_reward:.1f}', True, self.TEXT_PRIMARY)
        screen.blit(label_min, (self.x + 5, chart_origin_y - 10))

        latest_text = self._small_font.render(f'最终奖励: {rewards[-1]:.2f} | 平均: {avg_reward:.1f}', True, accent)
        ...
        current_text = self._small_font.render(f'当前轮累计: {current_episode_reward:.2f}', True, self.WARNING)

    ...
    def calculate_reward(self, prev_state, current_state):
        ...
        with self.server.data_lock:
            ...
            if min_distance < self.server.config_data.minSafeDistance and self.server.config_data.minSafeDistance > 0:
                collision_penalty = self.reward_config['collision_penalty'] * (self.server.config_data.minSafeDistance - min_distance) / self.server.config_data.minSafeDistance
        ...
        with self.server.data_lock:
            ...
            if runtime_data.leader_position is not None:
                ...
                if distance_to_leader > runtime_data.leader_scan_radius and runtime_data.leader_scan_radius > 0:
                    boundary_penalty = self.reward_config['boundary_penalty'] * (distance_to_leader - runtime_data.leader_scan_radius) / runtime_data.leader_scan_radius
        ...
        if prev_state is not None:
            ...
            # 计算方向变化的角度（余弦相似度）
            direction_change = 1 - np.dot(prev_direction, curr_direction) / (np.linalg.norm(prev_direction) * np.linalg.norm(curr_direction) + 1e-8)

...
1. 四组统一仿真对比阶段
- 用于组织 `ddpg_apf` 与 `pure_dqn` 的 stage01 训练，并统一衔接四组评测与分析
- 支持统一实验目录归档
- 支持继续训练建议输出
...
本版本将四组统一仿真对比阶段与虚实两阶段实验正式产品化为统一 workflow，补齐了实验状态管理、结果归档、继续训练建议和 CLI / 批处理入口，显著提升了实验流程的可重复性与可管理性。

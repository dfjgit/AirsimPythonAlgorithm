...
DOCX_PATH = OUTPUT_DIR / "TwoStage_Stage1_Stage2_Algorithms_v2.docx"
TEXT_PATH = OUTPUT_DIR / "TwoStage_Stage1_Stage2_Algorithms_v2.txt"
STAGE1_FLOWCHART = OUTPUT_DIR / "TwoStage_Stage1_Flowchart.png"
STAGE2_FLOWCHART = OUTPUT_DIR / "TwoStage_Stage2_Flowchart.png"


STAGE1 = {
    "title": "算法 4.3 双阶段实验第一阶段（sim_pretrain）训练流程",
    "known": (
        "已知：系统配置文件：C_sys，训练配置文件：C_train，奖励配置文件：C_reward，"
        "阶段一训练轮数：E_1，单轮最大步数：T_1，批大小：N，折扣因子：γ，软更新系数：τ。"
    ),
    "goal": "求：阶段一仿真预训练模型 M_sim 和阶段一训练日志 L_sim。",
    "lines": [
        {"num": 1, "text": "初始化演员网络参数 θ_μ 和评论家网络参数 θ_Q。", "indent": 0, "bold": False},
        {"num": 2, "text": "初始化目标演员网络和目标评论家网络：θ_μ' ← θ_μ，θ_Q' ← θ_Q。", "indent": 0, "bold": False},
        {"num": 3, "text": "初始化经验重放缓冲区 D ← ∅，初始化探索噪声过程 ψ，并启动 AirSim 训练环境 Env 与多无人机算法服务器 Server。", "indent": 0, "bold": False},
        ...
        {"num": 13, "text": "将经验样本 (s_t, ã_t, r_t, s_{t+1}, done) 存入经验池 D。", "indent": 2, "bold": False},
        {"num": 14, "text": "从 D 中随机采样 N 条经验，并计算目标值 y_i ← r_i + γQ'(s_{i+1}, μ'(s_{i+1}|θ_μ')|θ_Q')。", "indent": 2, "bold": False},
        {"num": 15, "text": "最小化损失函数更新评论家网络，并根据策略梯度更新演员网络。", "indent": 2, "bold": False},
        {"num": 16, "text": "软更新目标网络：θ_μ' ← τθ_μ + (1 − τ)θ_μ'，θ_Q' ← τθ_Q + (1 − τ)θ_Q'。", "indent": 2, "bold": False},
        ...
        {"num": 23, "text": "保存阶段一仿真预训练模型 weight_predictor_airsim、best_model 和阶段一训练日志。", "indent": 0, "bold": False},
...
STAGE2 = {
    ...
    "known": (
        "已知：阶段一输出模型：M_sim，后验更新模式：refine_mode ∈ {online, offline_logs}，"
        "真实飞行数据：D_real，双阶段实验输出目录：A_exp。"
    ),
    "goal": "求：阶段二后验更新模型 M_refine，双阶段汇总结果 S_2，继续修正建议 R。",
    "lines": [
        {"num": 1, "text": "读取阶段一输出模型 M_sim；若当前属于继续修正回合，则读取当前阶段二模型作为新的初始模型。", "indent": 0, "bold": False},
        {"num": 2, "text": "if refine_mode = online then", "indent": 0, "bold": True},
        {"num": 3, "text": "载入当前可用的阶段二初始模型，并采集新的实飞回合数据。", "indent": 1, "bold": False},
        {"num": 4, "text": "依据真实飞行回合结果执行在线后验更新。", "indent": 1, "bold": False},
        {"num": 5, "text": "保存当前回合更新后的模型和日志，记为当前阶段二输出 M_refine。", "indent": 1, "bold": False},
        {"num": 6, "text": "else if refine_mode = offline_logs then", "indent": 0, "bold": True},
        {"num": 7, "text": "读取已有实飞日志或后验数据集 D_real。", "indent": 1, "bold": False},
        {"num": 8, "text": "以阶段一模型 M_sim 为初始模型，利用真实日志完成离线后验更新。", "indent": 1, "bold": False},
        {"num": 9, "text": "保存当前回合更新后的模型和日志，记为当前阶段二输出 M_refine。", "indent": 1, "bold": False},
        {"num": 10, "text": "end if", "indent": 0, "bold": True},
        {"num": 11, "text": "将阶段二模型、日志和阶段元数据归档到 A_exp / real_weighted_refine / <mode>。", "indent": 0, "bold": False},
        {"num": 12, "text": "读取阶段一 sim_pretrain 与阶段二 real_weighted_refine 的训练统计结果。", "indent": 0, "bold": False},
        {"num": 13, "text": "构建 two_stage_summary.csv 和 two_stage_summary.md，得到双阶段汇总结果 S_2。", "indent": 0, "bold": False},
        {"num": 14, "text": "计算 efficiency gain 和 success gain。", "indent": 0, "bold": False},
        {"num": 15, "text": "if efficiency gain < 回退阈值 或 success gain < 回退阈值 then", "indent": 0, "bold": True},
        {"num": 16, "text": "R ← 谨慎继续修正。", "indent": 1, "bold": False},
        {"num": 17, "text": "else if efficiency gain > 显著提升阈值 或 success gain > 显著提升阈值 then", "indent": 0, "bold": True},
        {"num": 18, "text": "R ← 建议继续实飞修正。", "indent": 1, "bold": False},
        {"num": 19, "text": "else", "indent": 0, "bold": True},
        {"num": 20, "text": "R ← 当前可结束双阶段实验。", "indent": 1, "bold": False},
        {"num": 21, "text": "end if", "indent": 0, "bold": True},
        {"num": 22, "text": "若 R 表示继续修正，则当前 M_refine 作为下一轮阶段二的输入模型。", "indent": 0, "bold": False},

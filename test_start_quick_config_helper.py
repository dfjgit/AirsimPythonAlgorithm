...
class StartQuickConfigHelperTests(unittest.TestCase):
    def test_helper_writes_defaults_for_comparison_workflow_on_blank_input(self):
        ...
        try:
            ...
            self.assertIn("AIRSIM_QUICK_APF_BASELINE_EPISODES=100", content)
            self.assertIn("AIRSIM_QUICK_BENCHMARK_EPISODES=10", content)
            ...
            self.assertIn("APF 基线多轮仿真轮次 [默认: 100]", completed.stderr)
            self.assertIn("四组 benchmark 每 seed 评测轮次 [默认: 10]", completed.stderr)
    ...
    def test_helper_emits_english_copy_for_comparison_workflow(self):
        ...
        try:
            ...
            self.assertIn("APF Baseline Simulation Episodes [Default: 100]", completed.stderr)

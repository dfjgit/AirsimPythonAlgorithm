...
> 图 X 展示了 `DDPG+APF` 在 `stage02_finetune` 阶段中碰撞终止占比随训练轮次的变化情况。横轴为训练轮次，纵轴为碰撞终止占比（%）。该图用于评价训练过程中因碰撞失败的频繁程度，曲线越低通常表示训练稳定性越好。

### `collision_count_trend.png`

> 图 X 展示了 `DDPG+APF` 在 `stage02_finetune` 阶段中每轮碰撞次数随训练轮次的变化情况。横轴为训练轮次，纵轴为碰撞次数。该图用于观察追训过程中每轮碰撞负担是否下降，并辅助判断策略即使未因碰撞终止时是否仍存在较高碰撞代价。
...
> 图 X 展示了 `纯 DQN` 在 `stage02_finetune` 阶段中碰撞终止占比随训练轮次的变化情况。横轴为训练轮次，纵轴为碰撞终止占比（%）。该图用于评价训练过程中因碰撞失败的频繁程度，曲线越低通常表示训练稳定性越好。

### `collision_count_trend.png`

> 图 X 展示了 `纯 DQN` 在 `stage02_finetune` 阶段中每轮碰撞次数随训练轮次的变化情况。横轴为训练轮次，纵轴为碰撞次数。该图用于观察 DQN 追训后每轮碰撞负担是否下降，并辅助判断其训练稳定性是否随阶段推进而改善。
...
> 图 X 对比了 `DDPG+APF` 与 `纯 DQN` 在 `stage02` 阶段中的碰撞终止占比变化趋势。横轴为训练轮次，纵轴为碰撞终止占比（%）。该图主要用于比较两种算法的训练稳定性，辅助解释“谁更少因碰撞失败”。 

### `comparison_training_collision_count.png`

> 图 X 对比了 `DDPG+APF` 与 `纯 DQN` 在 `stage02` 阶段中的每轮碰撞次数变化趋势。横轴为训练轮次，纵轴为碰撞次数。该图用于比较两种算法在训练过程中的碰撞负担，并辅助判断碰撞代价是否随训练推进而下降。
